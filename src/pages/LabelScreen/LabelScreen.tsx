...
function LabelScreen(props: any) {

    const { id, label_name } = useParams()
    const idNum = id ? parseInt(id) : 0
    const navigate = useNavigate()

    const labelList = props.tasks?.filter((task: any) => {
        if(task.labels_id) {
            console.log(idNum)
            if(task.labels_id.includes(idNum)) return task;
        }
    })
    // console.log(labelList)

    const [taskList, setTaskList] = useState(labelList)
