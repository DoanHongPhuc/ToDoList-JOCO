...
import './duedate.css'
import DueDateDrapDown from "./DropDown";
...
function DueDate(props:any){
    ...
    const handleSaveClick = (str: string)=>{
        props.onChangeDueDate(str)
        setDueDateStr(str)
        setIsVisible(false)
    }
    ...
    return (
        <div className="task_editor_due_date relative">
            <button id="due_date_btn" onClick={(e)=>{e.preventDefault(); setIsVisible(!isVisible)}} className="task_editor_due_date_btn flex  items-center px-1.5 border border-gray-300 rounded text-xs text-gray-500 leading-7 hover:bg-gray-100">
            ...
            {isVisible && <DueDateDrapDown title = {dueDateStr === ''? 'Due date' : dueDateStr} handleSaveClick={handleSaveClick}/>}
        </div>
    )
}
